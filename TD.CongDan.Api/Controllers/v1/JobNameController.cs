...
namespace TD.CongDan.Api.Controllers.v1
{
    public class JobNameController : BaseApiController<JobNameController>
    {
        ...
        {
            var items = await _mediator.Send(new GetAllJobNamesQuery(pageNumber, pageSize, keySearch, orderBy));
            return Ok(items);
        }
        ...
        {
            var item = await _mediator.Send(new GetJobNameByIdQuery() { Id = id });
            return Ok(item);
        }
        ...
        public async Task<IActionResult> Post(CreateJobNameCommand command)
        {
        ...
        [HttpPut("{id}")]
        [Authorize(Roles = "Admin, SuperAdmin")]
        public async Task<IActionResult> Put(int id, UpdateJobNameCommand command)
        {
            if (id != command.Id)
            {
                return BadRequest();
        ...
        {
            return Ok(await _mediator.Send(new DeleteJobNameCommand { Id = id }));
        }
    }
}