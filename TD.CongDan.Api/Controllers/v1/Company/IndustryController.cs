...
namespace TD.CongDan.Api.Controllers.v1
{
    public class IndustryController : BaseApiController<IndustryController>
    {
        ...
        {
            var items = await _mediator.Send(new GetAllIndustriesQuery(pageNumber, pageSize, keySearch, orderBy));
            return Ok(items);
        }
        ...
        {
            var item = await _mediator.Send(new GetIndustryByIdQuery() { Id = id });
            return Ok(item);
        }
        ...
        public async Task<IActionResult> Post(CreateIndustryCommand command)
        {
        ...
        {
            return Ok(await _mediator.Send(new DeleteIndustryCommand { Id = id }));
        }
    }
}